  ...
  for (var row=(arena.row-1); row>=0; row--) {
    for (var column=0; column<arena.column; column++) {
      if (obj.food.x === column && obj.food.y === row) {
        drawning += `<div style="background-color: #E4BAD4; color: #AC66CC;"></div>`;
...
function updateSnakeBesties(json_from_erlang) {
  var obj = JSON.parse(json_from_erlang);
  // Parse Snake besties
  const besties = Object.entries(obj.best_players.elisnake_sm).map( 
    ([k,v]) =>  
...
function onMessage(evt) {
  ...
  switch (evt.data) {
    ...
    case (evt.data.match(/^(?=.*update)(?=.*elisnake_sm)/) || {}).input:
      updateSnakeBoard(evt.data);
      break;
    // Snake game best player
    case (evt.data.match(/^(?=.*best_players)(?=.*elisnake_sm)/) || {}).input:
